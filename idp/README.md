...
有一个 $ n*m $ 大小的蛋糕，上面有 $k$ 个樱桃，现在我们需要把这个蛋糕切成 $k$ 份，
使每份蛋糕上有一个樱桃，问最小切割长度和。(切割一刀必须切到底)

### 分析

用 $dp[i][j][k][l]$ 表示以 $(i, j)$ 为左上角，$(k, l)$ 为右下角的矩形切成每份一
个樱桃的最小切割长度。过 $(t,\; j) \; (i<t<k)$ 横切；过 $(i,\; s) \; (j<s<l)$ 纵
切可将该区间分成四个子区间。最终的结果即为子区间的结果加上两次切割的长度。
...
$ dp[a][b][c][d] = 0 $ 如果区间内没有樱桃。

**【状态转移公式】**

$$
 dp[i][j][k][l]=min(dp[i][j][t][l]+dp[t+1][j][k][l]+
                    dp[i][j][k][s]+dp[i][s+1][k][l]+
				    (l - j + 1)+(k - i + 1))
$$

## 刷字符串

给出字符串**A**和**B**，每一次我们可以使用一种颜色（用字母代替）刷任意一个连续子
序列，问将字符串**A**变成字符串**B**最少需要刷多少次。

### 分析

如果**A**是一个空字符串，则所需次数会最多。如果字符 $ A[i]=B[i] $，那么 $i$ 则可
被视为**A**与**B**的区间分割点。最终刷的次数为左右两个子区间次数之和。

**【最优子结构】**

$dp[i][j]$ 将一个空白串 $[i, j]$ 刷成**B**字符串对应位置的最小次数。
$ans[i]$ 将**A**字符串的前 $i$ 个字符，刷成**B**串对应字符需要的最小次数。

**【边界】**

$dp[i][j] = j - i + 1$ 假设**B**字符串每个字符均不相同。

$ans[0] = 0$

**【状态转移公式】**

$ dp[i][j]=min(dp[i][j], \; dp[i+1][k]+dp[k+1][j]),\qquad B[i] = B[k] $

$$
ans[i]=\left\{\begin{array}{ll}
min(ans[i],\; ans[i - 1]), \qquad &A[i]=B[i] \\
min(ans[i],\; ans[j] + dp[j + 1][i]), \qquad &A[i] \neq B[i],\; j < i \\
\end{array}
\right.
$$